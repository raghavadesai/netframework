...
class TCPServer:
    ...
    def start(self):
        ...
        try:
            ...
            self.running = True
            logger.info(f"Server started on {self.host}:{self.port} {'with SSL' if self.ssl_context else 'without SSL'}")
            ...
            while self.running:
                ...
                if self.ssl_context:
                    try:
                        ...
                        logger.info(f"SSL connection established with {address}")
                    except ssl.SSLError as e:
                        logger.error(f"SSL handshake failed with {address}: {e}")
                        client_socket.close()
                        continue
    ...
    def connect(self, host: str, port: int):
        ...
        try:
            ...
            if self.ssl_context:
                ...
                logger.info(f"SSL connection established with {host}:{port}")
